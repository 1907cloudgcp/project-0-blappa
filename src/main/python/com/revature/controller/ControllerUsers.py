...
class Functionalities:
    ...
    def save(self, connection, cursor):
        ...
        #get role ID
        role_id = ServiceRoles.Init().getId(roles_users_isbn_13, connection, cursor)

        user = Users(name, email, phnumber, birth_date, username, pswd, createdAt, updatedAt, enabled, role_id)
                    
        result = ServiceUsers.Init().create(user, connection, cursor)
    ...
    def login(self, username, pswd, connection, cursor):
        ...
        return ServiceUsers.Init().login(username, pswd, connection, cursor)
