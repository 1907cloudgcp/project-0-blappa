...
class Functionalities:
    ...
    def save(self, connection, cursor):
        ...
        #get user ID
        user_id = ServiceUsers.Init().getId(users_id, connection, cursor)
       
        account = Accounts(account_name, account_num, balance, typeA, description, createdAt, updatedAt, enabled, user_id)
                     
        result = ServiceAccounts.Init().create(account, connection, cursor)
    ...
    def deposit(self, connection, cursor):
        ...
        #make a deposit
        result = ServiceAccounts.Init().deposit(users_id, account, balance, description, createdAt, updatedAt, connection, cursor)
        startDate = createdAt
        endDate = updatedAt
        transaction = Transactions(account[1], description, typeTrs, startDate, endDate, createdAt, updatedAt, 1, account[0])

        result = ServiceTransactions.Init().save(transaction, connection, cursor)
    ...
    def withdraw(self, connection, cursor):
        ...
        #get user ID
        users_id = ServiceUsers.Init().getId(users_id, connection, cursor)
        
        #get account
        account = ServiceAccounts.Init().getAccount(account_num, connection, cursor)
        
        #make a deposit
        result = ServiceAccounts.Init().withdraw(users_id, account, balance, description, createdAt, updatedAt, connection, cursor)
        startDate = createdAt
        endDate = updatedAt
        transaction = Transactions(account[1], description, typeTrs, startDate, endDate, createdAt, updatedAt, 1, account[0])
    ...
    def view(self, acc_num, connection, cursor):
        ...
        #get account
        account = ServiceAccounts.Init().getAccount(acc_num, connection, cursor)
